...
def _parse_news_dataframe(df: pd.DataFrame, max_items: int = 100) -> list:
    ...
    # 컬럼명 확인 및 매핑 (다양한 CSV 포맷 지원) -------------------------------------------------- 실습 1 참고
    col_mapping = {
        'news_id': ['뉴스 식별자', '기사 고유번호', 'news_id', 'id'],
        'date': ['일자', 'date', '날짜'],
        'publisher': ['언론사', 'publisher', '매체'],
        ...
        with st.spinner("초기화 중..."):
            try:
                ...
                if not news_data:
                    ...
                    st.session_state.news_data = news_data # -------------------------------------- 실습 4 참고

                    # LLM 초기화
                    llm = create_llm()
                    st.session_state.llm = llm # --------------------------------------------------- 실습 4 참고
...
3. `semantic_search()` - 시맨틱 검색
4. `generate_rag_answer()` 적용 - RAG 생성 챗봇 구현
        ...
        if st.session_state.pending_response:
            ...
            with st.chat_message("assistant", avatar=AVATAR_BOT):
                with st.spinner("답변 생성 중..."):
                    try:                         
                        """generate_rag_answer()를 호출해 화면에 출력될 내용을 저장하세요.

                        - generate_rag_answer()는 dict를 반환합니다. 함수에서 반환하는 dict의 key 값을 확인하세요.
                        - 답변 생성을 위해선 어떤 정보가 들어가야 하는지를 파악하세요. (RAG의 기본 구조 참고)
                        - 답변에는 (1) RAG LLM이 생성한 답변과 (2) 참고한 뉴스의 출처 정보가 포함되어야 합니다.
                        
                        💡 힌트:
                        - 이전 실습에서 사용한 변수명이 무엇이었는지 확인하세요.
                        - 세션 상태에 저장되어 있는 변수들이 무엇인지와 변수명을 확인하세요.
                        - 반환된 dict에서 필요한 값을 key로 꺼내세요.
                        """
                        
                        # TODO: generate_rag_answer 호출 및 결과 추출
                        # ------------------------------------------------------------------------------------------------------------ 실습 4 정답 시작
